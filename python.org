...
4. 順番に色々入れていく：
   #+BEGIN_SRC sh
     ...
     sudo ARCHFLAGS=-Wno-error=unused-command-line-argument-hard-error-in-future pip install -U numpy # NumPy は Python で科学計算を行うための基本パッケージ

     brew install freetype
     ln -s ln -s /usr/local/opt/freetype/include/freetype2 /usr/local/include/freetype # http://stackoverflow.com/a/20576003/3506176
     sudo ARCHFLAGS=-Wno-error=unused-command-line-argument-hard-error-in-future pip install -U scipy # SciPy は科学・工学用モジュール群
      
     sudo pip isntall pyparsing # matplotlib のインストールに必要
     sudo ARCHFLAGS=-Wno-error=unused-command-line-argument-hard-error-in-future pip install -U matplotlib # matplotlib は Python で2Dグラフをプロットするためのライブラリ
     sudo pip install -U sympy # SymPy はシンボリックな数式処理のためのライブラリ

     # ネットワーク分析
     sudo pip install -U networkx    # NetworkX はネットワーク分析用パッケージ
     sudo ARCHFLAGS=-Wno-error=unused-command-line-argument-hard-error-in-future pip install -U graphillion # Graphillion は「数え上げお姉さん」を救うパッケージ
          
     # 画像処理
     sudo ARCHFLAGS=-Wno-error=unused-command-line-argument-hard-error-in-future pip install pillow        # Pillow は Python の画像処理ライブラリ
     ...
     # データ解析，自然言語処理
     sudo ARCHFLAGS=-Wno-error=unused-command-line-argument-hard-error-in-future pip install pandas nltk # pandas はデータ解析ライブラリ，nltk は自然言語処理ツールキット，
   #+END_SRC

5. パッケージ設定：
   Python で =site-packages= を探せるようにする：
   #+BEGIN_SRC sh
