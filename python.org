...
2. 環境変数 =GEOS_DIR= を作成
   ...
   #+END_SRC
3. =basemap-x.y.z= のダウンロード
   http://sourceforge.net/projects/matplotlib/files/matplotlib-toolkits/ から最新の =basemap-x.y.z.tar.gz= をダウンロード．

   ターミナルから =basemap-1.0.7.tar.gz= をダウンロードする場合，以下のように入力：
   #+BEGIN_SRC sh
     cd ~/Downloads/
     curl -LO http://sourceforge.net/projects/matplotlib/files/matplotlib-toolkits/basemap-1.0.7/basemap-1.0.7.tar.gz/download
   #+END_SRC
4. =basemap= をインストール
   #+BEGIN_SRC sh
     ARCHFLAGS=-Wno-error=unused-command-line-argument-hard-error-in-future \
         pip install --user -U ~/Downloads/basemap-1.0.7.tar.gz
   #+END_SRC
5. Python で以下のスクリプトを実行してみる
