...
1. 本体および関連するパッケージを Homebrew でインストール
   - ターミナルで以下を実行
      #+begin_src screen
        brew install emacs --cocoa --japanese # 最強のエディタ
        brew linkapps # emacs へのシンボリックリンクを /Applications フォルダ下に作成
        brew install magit                                              # Emacs 上の git フロントエンド
...
Macの標準入力方式に代替できる [[http://aquaskk.sourceforge.jp][AquaSKK]] と Emacs 上で動く [[http://openlab.ring.gr.jp/skk/ddskk-ja.html][DDSKK (Daredevil SKK)]] とがあり，両方併用するのが吉．
** COMMENT DDSKKのインストール
1. Emacs をインストールし終わっていて，
   =/Applications/Emacs.app/Contents/MacOS/Emacs= が有効になっている必要がある．
2. 長江自作の Formula を用いれば Homebrew でインストール可能：
   #+BEGIN_SRC screen
   $ brew install https://gist.github.com/nagae/3009185/raw/ddskk.rb
   #+END_SRC
** AquaSKKのインストール
1. http://aquaskk.sourceforge.jp/ の右側の「ダウンロード」の欄の「AquaSKK パッケー
   ジ」から AquaSKK-4.1.dmg をダウンロードし，AquaSKK.pkg を実行．
2. Mountain Lion の Gatekeeper が邪魔をする場合は，Ctrlを押しながらクリック→開く．
3. 言語環境に AquaSKK を登録
