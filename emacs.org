...
2. 長江自作の Formula を用いれば Homebrew でインストール可能：
   ...
   #+END_SRC
** AquaSKKのインストール
1. https://github.com/codefirst/aquaskk/releases から最新のバージョン (2015/4/6時点では AsuaSKK-4.2.4.dmg) をダウンロードし，AquaSKK.pkg を実行．
2. Mac OS X の Gatekeeper が邪魔をする場合は，Ctrlを押しながらクリック→開く．
3. 言語環境に AquaSKK を登録
