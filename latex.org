...
6. TeX フォルダパスの設定
   ...
   #+BEGIN_SRC sh
     ...
     export MANPATH=/usr/local/texlive/2014/texmf/doc/man:$MANPATH
     export INFOPATH=/usr/local/texlive/2014/texmf-dist/doc/info:$INFOPATH
   #+END_SRC

   TeXLive 2013 で既に =PATH= などを設定済みなら，ターミナルから以下を実行すればよ
   い．
   #+BEGIN_SRC sh
   sed 's|/usr/local/texlive/2013|/usr/local/texlive/2014|g' ~/.bash_profile
   #+END_SRC
